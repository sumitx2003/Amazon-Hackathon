...
export default function ReturnPortal({ currentUser, session }) {
  ...
  const handleEvaluateSubmission = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:5000/api/marketplace/process-return", {
        ...productData,
        sellerId
      });
  ...
  const handleAcceptAndDisburse = async () => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:5000/api/marketplace/finalize-agreement", payload);
      if (response.data.success) {
