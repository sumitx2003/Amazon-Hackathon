...
export default function AdminDashboard({ onCreditsMinted }) {
  ...
  const processAction = async (id, action) => {
    try {
      const res = await axios.put(`http://localhost:5000/api/marketplace/admin-decision/${id}`, { action });
      if (res.status === 200) {
  ...
  const handleTriggerBulkAiAgent = async () => {
    setAiLoading(true);
    try {
      const res = await axios.post('http://localhost:5000/api/marketplace/bulk-ai-audit');
      if (res.data.success) {
  ...
  const handleCommitBulkChoices = async () => {
    ...
    try {
      ...
      for (const item of aiSuggestions) {
        const res = await axios.put(`http://localhost:5000/api/marketplace/admin-decision/${item._id}`, { 
          action: item.action 
        });
